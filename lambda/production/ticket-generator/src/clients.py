...
def get_elasticsearch_client():
    """
    SSM 및 Secrets Manager 정보를 이용하여 Elasticsearch 클라이언트 생성 (Singleton)
    """
    global _es_client
    ...
    try:
        ...
        # 1. 접속 정보 로드
        es_host = ssm.get_parameter(Name=config.SSM_ES_HOST_PARAM)["Parameter"]["Value"]
        es_user = ssm.get_parameter(Name=config.SSM_ES_USER_PARAM)["Parameter"]["Value"]
        es_password = ssm.get_parameter(Name=config.SSM_ES_PASS_PARAM, WithDecryption=True)["Parameter"]["Value"]
        ...
        if _es_client.ping():
            ...
            print("[WARN] Elasticsearch Ping 실패")
