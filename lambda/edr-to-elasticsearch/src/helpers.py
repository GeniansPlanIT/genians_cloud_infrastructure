...
def transform_for_ai_pipeline(event_doc):
    """edr-ai-pipeline.conf의 필터 로직을 수행하는 함수"""
    
    # 1. 원본 복사하여 작업
    doc = event_doc.copy()

    # 2. 날짜 필드 변환 (Logstash의 date 필터 역할)
    # 원본 @timestamp가 없을 때만 DetectTime으로 생성하도록 로직 변경
    ...
    whitelist = {
        ...
        "SuspiciousInfo", "SuspiciousInfo2", "SuspiciousInfo3", "TrunkID", "ChildTrunkID", "ResponseInfo", "PathInfo", "PathInfo2", "DetectDesc", "DetectType", "DetectID"
    }
